...
export const SupervisorInput = () => {
  ...
  const exportNakerToExcel = async () => {
    ...
    try {
      ...
      if (!res.success || !res.rows || res.rows.length === 0) {
        alert('Tidak ada data NAKER untuk diekspor.');
        return;
      }
  ...
  const exportQsfToExcel = async (channelName) => {
    ...
    try {
      ...
      if (!res.success || !res.rows || res.rows.length === 0) {
        alert(`Tidak ada data assessment ${channelName} untuk diekspor.`);
        return;
      }
      const ws = XLSX.utils.json_to_sheet(res.rows);
      const wb = XLSX.utils.book_new();
  ...
  const submitImport = async () => {
    ...
    const BATCH_SIZE = 50; // Ukuran batch per request (50 baris) agar memory ringan dan anti-crash
    const chunks = [];
  ...
  const handleResetDatabase = async () => {
    if (window.confirm('PERINGATAN: Apakah Anda yakin ingin mengosongkan SELURUH data nilai penilaian agen di database MySQL?')) {
      try {
        await api.resetSystemData();
        triggerDataUpdate();
        fetchSummary();
        if (activeTab === 'data') fetchAgentsData();
        alert('Seluruh data penilaian agen di database berhasil dikosongkan!');
      } catch (err) {
    ...
    <div className="space-y-4 sm:space-y-6">
        ...
        {IMPORT_TYPES.map(ch => {
          ...
          return (
            <div
              ...
              className={`p-3 rounded-2xl border transition cursor-pointer relative overflow-hidden flex flex-col justify-between active:scale-95 select-none ${
                (selectedChannel === ch.id || (isNakerCard && activeTab === 'naker'))
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <div className={`w-7 h-7 rounded-xl flex items-center justify-center ${
                    ch.color === 'blue' ? 'bg-blue-50 text-blue-700' :
                  ...
                  <span className={`px-1.5 py-0.2 text-[8px] font-bold rounded ${
                    isNakerCard ? 'bg-blue-50 text-blue-800 border border-blue-200' :
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-2 overflow-x-auto scroll-smooth">
        <button
          ...
          className={`px-3.5 py-2 rounded-xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap active:scale-95 shadow-2xs ${
            activeTab === 'import'
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50 text-slate-700 font-bold sticky top-0 uppercase tracking-wider text-[10px] border-b border-slate-200">
                      <tr>
                        ...
                        <th className="p-2.5">Layanan Target</th>
                        ...
                        <tr key={item.row_index} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="p-2.5">
                            <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold border ${
                              item.status === 'valid' ? 'bg-emerald-50 text-emerald-800 border-emerald-300' :
                          ...
                          <td className="p-2.5">{item.gender}</td>
                          <td className="p-2.5 font-medium text-slate-700">{item.layanan}</td>
                          <td className="p-2.5 text-slate-600">{item.team_tl}</td>
                          <td className="p-2.5 text-slate-600">{item.trainer}</td>
                          <td className="p-2.5 font-bold">{item.site}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {previewResult.items?.map((item) => (
                        <tr key={item.row_index} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="p-2.5">
                            <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold border ${
                              item.row_type === 'update' ? 'bg-amber-50 text-amber-800 border-amber-300' :
                          ...
                          <td className="p-2.5 text-center">
                            <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold ${
                              (item.ca ?? item.score_ca) >= 96 ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' :
                  ...
                  agentsList.map((agent, index) => (
                    <tr key={agent.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold ${
                          agent.status === 'Exceed Target' ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' :
      ...
      {activeTab === 'naker' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="corp-card p-3.5 flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-700 flex items-center justify-center font-bold">
                <Users className="w-5 h-5" />
              </div>
              <div>
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Total NAKER</span>
                <span className="text-xl font-black text-slate-900 leading-none">{nakerSummary.total_naker || nakerList.length}</span>
                <span className="text-[10px] text-slate-500">Personel Terdaftar</span>
              </div>
            </div>
            ...
            <div className="corp-card p-3.5 flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Tenaga Kerja Pria</span>
                <span className="text-xl font-black text-indigo-900 leading-none">{nakerSummary.pria || 0}</span>
                <span className="text-[10px] text-slate-500">Laki-Laki (L)</span>
              </div>
            </div>
            ...
            <div className="corp-card p-3.5 flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Tenaga Kerja Wanita</span>
                <span className="text-xl font-black text-pink-900 leading-none">{nakerSummary.wanita || 0}</span>
                <span className="text-[10px] text-slate-500">Perempuan (P)</span>
              </div>
            </div>
            ...
            <div className="corp-card p-3.5 flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Lokasi Kerja</span>
                <span className="text-xl font-black text-emerald-900 leading-none">SMG</span>
                <span className="text-[10px] text-slate-500">Site Semarang</span>
              </div>
            </div>
          </div>
              ...
              <table className="w-full text-left text-xs border-collapse min-w-[800px]">
                <thead className="bg-slate-50 text-slate-700 font-bold text-[11px] uppercase tracking-wider border-b border-slate-200">
                  <tr>
                    ...
                    <th className="py-3 px-4">ID SIP</th>
