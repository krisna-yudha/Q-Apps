...
export const SupervisorInput = () => {
    ...
    const [activeTab, setActiveTab] = useState(isTLorTrainer ? 'naker' : 'import'); // 'import', 'naker', 'manual', 'data', 'history'
    ...
    return (
        <div className="space-y-4 sm:space-y-6">
            {/* Header & Role Supervisor / TL Badge */}
            <div className="corp-card p-4 sm:p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-3.5 sm:gap-4">
                <div>
                    <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border flex items-center gap-1 ${
                            ...
                            {isSupervisor ? 'SUPERVISOR CONTROL HUB' : 'MONITORING MASTER NAKER'}
                        </span>
                        <h1 className="text-base sm:text-xl font-bold text-slate-900 tracking-tight">
                            {isSupervisor ? 'Pusat Import & Input Data' : 'Data Master & Plotting Penugasan'}
                        </h1>
                    </div>
                    {!isSupervisor && (
                        <p className="text-xs text-slate-500 mt-1">
                            Mode Pemantauan Database NAKER & Plotting Binaan (Read-Only).
                        </p>
                    )}
                </div>
                ...
                <div className="flex items-center gap-2 w-full lg:w-auto">
                    {isSupervisor ? (
                        ...
                        <Link
                            to="/rekap-under-team"
                            className="w-full sm:w-auto px-3.5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-2xs active:scale-95"
                        >
                            <Users className="w-3.5 h-3.5" />
                            <span>Buka Rekap Tim Binaan</span>
                        </Link>
                    )}
                </div>
            </div>
